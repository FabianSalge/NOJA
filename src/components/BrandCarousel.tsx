			...
			<div className="relative h-16 md:h-20">
				<div className="absolute top-0 left-0 flex gap-14 opacity-80 marquee-ltr" style={{ width: '200%' }}>
					<div className="flex gap-14 w-1/2 justify-around">
						{items.map((src, idx) => (
							<img key={`brand-${idx}`} src={src} alt="brand logo" className="h-12 md:h-16 w-auto object-contain" />
						))}
					</div>
					<div className="flex gap-14 w-1/2 justify-around">
						{items.map((src, idx) => (
							<img key={`dup-brand-${idx}`} src={src} alt="brand logo" className="h-12 md:h-16 w-auto object-contain" />
						))}
					</div>
				</div>
			</div>
