      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 w-full">
        <motion.div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center" variants={containerVariants} initial="hidden" animate={storyInView ? 'visible' : 'hidden'}>
          ...
          <motion.div className="relative" variants={itemVariants} transition={{ duration: 0.6, ease: 'easeOut' }}>
            <motion.div className="aspect-square bg-gradient-to-br from-[hsl(var(--primary))]/60 to-[hsl(var(--primary))]/80 rounded-3xl overflow-hidden border border-background/10 shadow-2xl max-w-xs sm:max-w-md mx-auto" whileHover={{ y: -8 }} transition={{ duration: 0.3, ease: 'easeOut' }}>
              {imageUrl ? (<img src={imageUrl} alt="Team collaboration" className="w-full h-full object-cover hidden md:block" />) : null}
            </motion.div>
          </motion.div>
        </motion.div>
      </div>
